...
class INIParser(object):
    ...
    def parse(self, ini):
        ...
        for section in meta.sections():
            ...
            # we have a section of type [webanchor.en]
            if len(section) > 2 and section.find('.') > -1:
                (chapter, lang) = section.rsplit(".", 1)
                langmap = metadata.get(lang, {})
                langmap[chapter] = _getMeta(meta.items(section))
                metadata[lang] = langmap
            else:
                # section is a language abbrev like [en] ad therefore
                # a main section
                lang = section
                langmap = metadata.get(lang, {})
    ...
    def retrieve(self, context):
        """ retrieves the metadata from the object, all translations and
        all chapters
        """
        meta = ConfigParser.ConfigParser()
        meta.optionxform = str
